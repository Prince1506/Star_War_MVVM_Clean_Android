...
import com.mvvm_clean.user_details.core.domain.functional.onSuccess
import com.mvvm_clean.user_details.features.common.domain.api.StarWarApiRepository.Network
...
class StarWarApiRepositoryTest : UnitTest() {
    ...
    fun `should return empty planet list by default`() {
        ...
        every { service.getLoginInfo(PLANET_ID) } returns planetListResponseCall

        //Act
        val planetEntity = networkRepository.getLoginInfo(PLANET_ID)

        //Verify
        planetEntity shouldEqual Right(planetListEntity.toPlanetsDataModel())
        verify(exactly = 1) { service.getLoginInfo(PLANET_ID) }
    }
    ...
    fun `planet list api should return server error if no successful response`() {
        ...
        every { service.getLoginInfo(PLANET_ID) } returns planetListResponseCall
        //Act
        val planets = networkRepository.getLoginInfo(PLANET_ID)
        //Verify
    ...
    fun `should get planet list from api`() {
        ...
        every { service.getLoginInfo(PLANET_ID) } returns planetListResponseCall
        //Act
        val peopleListResponseEntity = networkRepository.getLoginInfo(PLANET_ID)
        //Verify
